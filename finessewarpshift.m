function shift = finessewarpshift(shiftOld)
% FINESSEWARPSHIFT processes SHIFT (see GENERATEWARPSHIFT). SHIFT is a
% vector of shift values. Where no shift was identified in
% GENERATEWARPSHIFT then SHIFT=NAN. In this function the NAN values are
% filled in.
% 
%
%
...
% code
% ---------------------------------------------------------------
    % Initialization and Gradient Control:
    
    shift = shiftOld;
    
    % First identify the start and end indices of existing data
    shift([1,end]) = nan; % force first and last value to nan to prevent edge effects when interpolating.

    % Remove sections with steps or big changes in gradient that exceeds 1.
    % diff(X) calculates differences between adjacent elements of X. If X has m elements, diff(X) will have m-1 elements.
    dShift = [0 ; diff(shift)]; % 0 prepended so the dShift pins onto the 
    shift(dShift>1) = nan;      % secondvalue that makes up the gradient.



    % Identifying Valid Segments:
    % iStart and iFinish identify the starting and ending indices of valid sections (non-NaN segments).

    isVal = ~isnan(shift); % Logical array marking indices of shift where the value is not NaN.
    
    if ~any(isVal); shift = zeros(size(shift)); return; end % If all elements of shift are NaN, return a vector of all zeros. 

    isValAfter = [isVal(2:end);false]; % This shifts the content of isVal by one index toward the start and appends false at the end.
    isValBefore = [false; isVal(1:(end-1))]; % This shifts the content of isVal by one index toward the end and appends false at the start.

    iStart  = find(isVal & ~isValBefore); % Logical array marking indices where the current element is valid but the previous element is NaN.
    iFinish = find(isVal & ~isValAfter); % Logical array marking indices where the current element is valid but the next element is NaN.

    % First, we want to check that we could join the finish of each section to
    % the start of the next without exceeding a gradient of +/- 1
    % If the difference in shift between the start of a new section and the end of the previous section (shiftDiff) exceeds the time 
    % difference (tDiff) between them, interpolation would introduce a gradient greater than 1. 
    % In such cases, trim the valid segments at both ends of the gap by a calculated amount (trim) to reduce the gradient.
    % If there isn’t enough data to trim effectively, the end values of the valid segments to either sides of the gap are marked as NaN.
    for i = (1:(numel(iFinish)-1)) %go through all the blanks in turn

	% Time difference across the gap (in indices)
        tDiff = iStart(i+1) - iFinish(i);   if tDiff<0; error(); end
        shiftDiff = shift(iStart(i+1)) - shift(iFinish(i));

        if shiftDiff>tDiff %then the gradient is changing too fast
            
            trim = ceil((shiftDiff - tDiff)/2);
            % We need to chop this amount of data off each side of the 
            % blank, but we can't if there isn't enough data.

            lengthBefore = iFinish(i) - iStart(i);
            lengthAfter = iFinish(i+1) - iStart(i+1); % Previously set to iFinish(i) - iStart(i) by mistake?

            if lengthBefore > trim
                iFinish(i) = iFinish(i)-trim; 
            else
                iStart(i) = nan;
                iFinish(i) = nan;
            end
    ...
    iStart(isnan(iStart)) = [];
    iFinish(isnan(iFinish)) = [];
    
    % Now fill in the bits between data using linear interpolation.
    % Create first and last points on 'zero order hold'
    iStart = [1 ; iStart ; numel(shift)];
    iFinish = [1 ; iFinish ; numel(shift)];
    shift(iStart(end)) = shift(iStart(end-1));
    shift(iFinish(1)) = shift(iFinish(2));

    
    for i = (1:(numel(iFinish)-1)) %go through all the blanks in turn again
        % Time difference across the gap (in indices)
        tDiff = iStart(i+1) - iFinish(i);   if tDiff<0; error(); end
        shiftDiff = shift(iStart(i+1)) - shift(iFinish(i));
        
        shift(iFinish(i)+(1:tDiff)) = ...
                            shift(iFinish(i)) + (1:tDiff)* shiftDiff/tDiff;
