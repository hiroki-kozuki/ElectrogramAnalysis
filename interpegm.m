function eInterp = interpegm(pNew, tri, egm, sampleFreq, noiseLevel)
% INTERPEGM generates an interpolated electrogram that is equivalent to 
% eInterp <=> eX + f*(eY-eX). It uses time warping given by SHIFTXY
% Usage:
%   eInterp = interpegm(eX, eY, shiftXY, f)
% Inputs:
%   eX,eY  - input electrograms
%   shiftXY - the shift from eX to eY at each index of eX 
%         i.e. eX(iX) <-> eY(iX+shift(iX) (indices, but may be non-integer)
%   f  - as defined above, 0<f<1, can be a vector
% Outputs:
%   eInterp  - output
%
...
% ---------------------------------------------------------------
    ...
    % compare electrograms along each edge
    triEdges = tri.edges;
    ...
    for iEdge = 1:nEdges
        ...
        iEdge
        shift(:,iEdge) = egmtimewarp(eX,eY,sampleFreq, tWindowWidth, tMaxLag, noiseLevel);
    end
    ...
    for iT = 1:size(tri,1) % for each triangle
        ...
        iEdges = nan(3,1);
        for i = 1:3
            test1 = tEdges(i,1)==triEdges(:,1);
            test2 = tEdges(i,2)==triEdges(:,2);
            k = find( test1 & test2 ,1);
            if isempty(k); error(); end
            iEdges(i) = k;
        end
        
        eInterpBlock = local_interp(tBary, egm(:,tPoints), tPoints, triEdges(iEdges,:), shift(:,iEdges));
        eInterp(:,ind) = eInterpBlock;
...
function eInterp = local_interp(lambda, egm, points, edges, shifts)
    ...
    % unpacking to make code easier to read
    e1 = egm(:,1);
    e2 = egm(:,2);
    e3 = egm(:,3);
    ...
    % we must be very careful to get things the right way round
    ...
    for j = 1:2
        for i = 1:3
            ...
            if isequal(edge13,edges(i,:))
                shift13 = shifts(:,i);
            end
            if isequal(edge23,edges(i,:))
                shift23 = shifts(:,i);
            end
        end
        % also try reversing the edges
        edges = edges(:,[2,1]);
        %and the signs
        shifts = -shifts;
    end
    
    if isempty(shift12); error(); end %#ok<LTARG>
    if isempty(shift13); error(); end %#ok<LTARG>
    if isempty(shift23); error(); end %#ok<LTARG>
    % if these errors throw, then we need to change the order of the edges
    % and use -shifts(:,i) instead of +shifts(:,i)
    ...
    % first get the shifts right
    ind1 = ind - shift12 * lambda2 - shift13 * lambda3;
    ind1 = local_clean(ind1,nE);
    e1onLambda = e1(ind1);
    
    ind2 = ind + shift12 * lambda1 - shift23 * lambda3;
    ind2 = local_clean(ind2,nE);
    e2onLambda = e2(ind2);
    
    ind3 = ind + shift13 * lambda1 + shift23 * lambda2;
    ind3 = local_clean(ind3,nE);
    e3onLambda = e3(ind3);
